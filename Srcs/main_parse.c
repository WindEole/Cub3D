...
static int	ft_count_elm(t_adm **adm, t_dll *now, int y, int *v)
{
	...
	// v[3] = 0;
	v[0] = -1;
printf(CYAN"On entre ici ? main_parse/ ft_count_elm"RESET"\n");
	while (++v[0] < y)
	{
		now = adm[v[0]]->head;
printf(YELLOW"now->c = [%c]"RESET"\n", now->c);
		while (now != NULL)
		{
printf(GREEN"now->c = [%c]"RESET"\n", now->c);
			if ((now->c == 'N' || now->c == 'S' || now->c == 'E' || now->c == 'W') && v[1]++ != -1)
				adm[0]->play = now;
			// else if (now->c == 'E')
			// 	v[2]++;
			// else if (now->c == 'C')
	...
	adm[0]->col = 1; // A SUPPRIMER POUR LA SUITE
//	adm[0]->col = v[3];
printf(YELLOW"v1 = [%d]"RESET"\n", v[1]);
